...
const appendDevBrowserOnCrossOrigin = (req, res) => {
  ...
  if (shouldAppendDevBrowser && !!location && (0, import_utils2.isDevelopmentFromApiKey)(import_clerkClient.SECRET_KEY) && (0, import_utils2.isCrossOrigin)(req.experimental_clerkUrl, location)) {
...
const assertInfiniteRedirectionLoop = (req, res) => {
  if (!(0, import_utils2.isDevelopmentFromApiKey)(import_clerkClient.SECRET_KEY)) {
    return res;
  }
...
-> Make sure you are using the latest version of '@clerk/nextjs' and 'next'.
  `;
const getFirstValueFromHeader = (req, key) => {
  const value = req.headers.get(key);
  return value?.split(",")[0];
};
const withNormalizedClerkUrl = (req) => {
  if (!import_clerkClient.TRUST_HOST) {
    return Object.assign(req, { experimental_clerkUrl: req.nextUrl });
  }
  const clerkUrl = req.nextUrl.clone();
  clerkUrl.protocol = getFirstValueFromHeader(req, import_backend.constants.Headers.ForwardedProto) ?? clerkUrl.protocol;
  clerkUrl.host = getFirstValueFromHeader(req, import_backend.constants.Headers.ForwardedHost) ?? clerkUrl.host;
  clerkUrl.port = getFirstValueFromHeader(req, import_backend.constants.Headers.ForwardedPort) ?? clerkUrl.port;
